...
function validateDatabase(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    datosSeparados.push(data[i].split(","));
  }

  for (let i = 0; i < datosSeparados.length; i++) {
    for (let j = 0; j < datosSeparados[i].length; j++) {
      if (j == 0) {
        if (
          datosSeparados[i][j] == "" ||
          datosSeparados[i][j].match(reExSimbolos) != null
        ) {
          // console.log("ID invalida: ", datosSeparados[i][j]);
          valoresInvalidados.push(datosSeparados[i][1]);
        }
      }
      if (j == 1) {
        if (
          ...
          // console.log("Username invalido: ", datosSeparados[i][j]);
          valoresInvalidados.push(datosSeparados[i][1]);
        }
      }
      if (j == 2) {
        if (
          datosSeparados[i][j] == "" ||
          datosSeparados[i][j].match(".com") == null ||
          datosSeparados[i][j].match(emailRegex) == null
        ) {
          // console.log("email invalido:", datosSeparados[i][j]);
          valoresInvalidados.push(datosSeparados[i][1]);
        }
      }
      if (j == 3) {
        if (datosSeparados[i][j].match(regExNumeros) == null) {
          // console.log("Edad invalida: ", datosSeparados[i][j]);
          valoresInvalidados.push(datosSeparados[i][1]);
        }
      }
      if (j == 4) {
        if (datosSeparados[i][j].match(regExLetras) == null) {
          // console.log("Edad invalida: ", datosSeparados[i][j]);
          valoresInvalidados.push(datosSeparados[i][1]);
        }
      }
  ...
  let result = valoresInvalidados.filter((item, index) => {
    return valoresInvalidados.indexOf(item) === index;
  });
...
console.log(validateDatabase(database));