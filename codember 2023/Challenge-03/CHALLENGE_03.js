...
function invalidKeys(list) {
  let rangos = [];
  let letra = [];
  let contador = 0;
  let contadorInvalido = 0;
  let resultado = "";
  let result = "";
  let invalido = [];
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let rangoParte = partes[0];
    let valores = rangoParte.split("-");
    let rangoInicio = parseInt(valores[0]);
    let rangoFin = parseInt(valores[1]);
    let simbolo = partes[1][0];
    rangos.push({ inicio: rangoInicio, fin: rangoFin, signo: simbolo });
  }

  for (let i = 0; i < arr.length; i++) {
    for (let j = 7; j < arr[i].length; j++) {
      if (arr[i][j] == rangos[i].signo) {
        contador++;
      }
      if (contadorInvalido == 41) {
        result += arr[i][j];
      }
    }

    if (contador >= rangos[i].inicio && contador <= rangos[i].fin) {
      letra.push(rangos[i].signo);
      resultado += rangos[i].signo;
      contador = 0;
    } else {
      invalido.push(arr[i]);
      contadorInvalido++;
  ...
  //   console.log(invalido[41]);
  //   console.log(resultado);
  return result;
}

console.log(invalidKeys(arr));
